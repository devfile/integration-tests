...
var _ = Describe("odo devfile config command tests", func() {
	...
	When("a component is created", func() {
		...
		When("executing config set an environment variable", func() {
			const (
				KAFKA_HOST    = "kafka"
				KAFKA_PORT    = "6639"
			)
			...
			BeforeEach(func() {
				helper.Cmd("odo", "config", "set", "--env", "KAFKA_HOST=" + KAFKA_HOST +
					",KAFKA_PORT=" + KAFKA_PORT).ShouldPass()
			})

			It("should successfully set an environment variable", func() {
				output := helper.Cmd("odo", "config", "view").ShouldPass().Out()
				wantOutput := []string{
					KAFKA_HOST,
					KAFKA_PORT,
			...
			When("unsettting an environment variable", func() {
				BeforeEach(func() {
					helper.Cmd("odo", "config", "unset", "--env", "KAFKA_HOST", "--env", "KAFKA_PORT").ShouldPass()
				})
